...
const ModelTraining = () => {
  const { dataset, setModelData } = useAppContext();
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
        <div className="bg-slate-800 rounded-xl p-8 border border-slate-700">
          <form onSubmit={handleTrain} className="space-y-6">
            <div>
              <label className="block text-sm font-medium text-slate-300 mb-2">Target Variable</label>
              <select 
                value={target}
        ...
        {results && (
          <div className="bg-slate-800 rounded-xl p-8 border border-slate-700">
            ...
            <div className="flex items-center gap-4 mb-6">
              ...
              <div className="text-slate-400">Baseline<br/>Accuracy</div>
            </div>
            
            <div>
              <h3 className="text-sm font-medium text-slate-400 mb-2">Confusion Matrix</h3>
              <div className="grid grid-cols-2 gap-2 text-center text-sm font-mono">
                {results.confusion_matrix.map((row, i) => (
                  row.map((cell, j) => (
                    <div key={`${i}-${j}`} className="bg-slate-900 p-3 rounded border border-slate-700">
                      {cell}
                    </div>
                  ))
                ))}
